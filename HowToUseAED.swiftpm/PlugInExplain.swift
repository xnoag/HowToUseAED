//
//  PlugInExplain.swift
//  HowToUseAED
//
//  Created by xnoag on 2023/04/16.
//

import SwiftUI

import SwiftUI
import AVFoundation
import AVKit


import SwiftUI
import AVFoundation
import AVKit

struct PlugInExplain: View {
    @State var audioPlayer: AVAudioPlayer!
    let buttonClickSound1 = Bundle.main.path(forResource: "ButtonClickSound", ofType: "mp3")
    var body: some View {
        NavigationView {
            GeometryReader { geo in
                ZStack {
                    Image("PlugInView")
                        .resizable()
                        .scaledToFit()
                        .opacity(0.05)
                        .frame(width: geo.size.width * 1, height: geo.size.height * 0.9)
                        .background(Color.white)
                    VStack {
                        Spacer()
                            .frame(width: geo.size.width * 1, height: geo.size.height * 0.3)
                        VStack(alignment: .leading) {
                            Text("Great!")
                                .font(.largeTitle)
                                .fontWeight(.bold)
                                .padding(.bottom, 10)
                            Text("When you connect the wires attached to the pads to the machine,\rit can automatically analyze the heart rhythm.")
                                .font(.title)
                                .fontWeight(.bold)
                            .foregroundColor(Color.black)
                        }
                            .frame(width: geo.size.width * 1, height: geo.size.height * 0.3)
                        Spacer()
                            .frame(width: geo.size.width * 1, height: geo.size.height * 0.2)
                        HStack {
                            Spacer()
                                .frame(width: geo.size.width * 0.7, height: geo.size.height * 0.2)
                            NavigationLink(destination: AnalyzeHeartRythm()) {
                                Text("Next Step")
                                    .foregroundColor(.white)
                                    .font(.title)
                                    .fontWeight(.bold)
                                    .padding(.horizontal, 30.0)
                                    .padding(.vertical, 15)
                                    .background(Color.red)
                                    .cornerRadius(10)
                            }//NavigationLink
                        }//HStack
                    }//VStack
                }//ZStack
                .padding()
                .onDisappear() {
                    self.audioPlayer = try! AVAudioPlayer(contentsOf: URL(fileURLWithPath: buttonClickSound1!))
                    audioPlayer?.play()
                }
            }//GeometryReader
        }//NavigationView
        .navigationViewStyle(StackNavigationViewStyle())
        .navigationBarHidden(true)
    }
}

struct PlugInExplain_Previews: PreviewProvider {
    static var previews: some View {
        PlugInExplain()
            .previewDevice(PreviewDevice(rawValue: "iPad Pro (12.9-inch)"))
            .previewInterfaceOrientation(.landscapeRight)
    }
}
